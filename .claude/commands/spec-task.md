...
argument-hint: Brief description of the feature idea (e.g., "combined review queue", "gitlab mr sync")
---
...
> You're already inside a task worktree. `/spec-task` must be run from the main repo root (the directory `git rev-parse --git-common-dir` resolves to, minus the trailing `.git`). `cd` there and re-run.

### Step 2 — Determine task number and working slug

1. Run `tools/task-numbers.sh next` to get the next free `NNN` (zero-padded, three digits). This is the single source of truth: it scans `docs/tasks/` on main, every `.worktrees/*/docs/tasks/` folder, and local `task-*` branches, so tasks-in-flight reserve their numbers even if not yet on main. Do NOT pick a number by hand. The very first task is `001`.
2. Derive a working slug from `$ARGUMENTS` (lowercase, hyphenated, 3–4 words). Examples: "combined review queue" → `combined-review-queue`, "gitlab mr sync" → `gitlab-mr-sync`.
...
1. Read project docs in `docs/` and any relevant service docs in `apps/*/docs/` that the idea touches. (These may not exist yet for an early task — that's fine, note it.)
...
From this point on, every file write, read, and shell command MUST use absolute paths under the worktree. Resolve it once with `git worktree list` and reuse that path (e.g., `<main-repo-root>/.worktrees/task-NNN-<slug>/docs/tasks/task-NNN-<slug>/prd.md`). Do NOT write any files under the main repo's `docs/tasks/`.
